...
# Class
class PostgreSQLJob(BaseJob):
    def __init__(self, job):
        super().__init__(job)

        self.name = f'postgresql-{job.name}'
        self._type = 'PostgreSQL'
        self._server = job.config['postgresql']['server']
        self._port = job.config['postgresql']['port']
        self._username = job.config['postgresql']['username']
        ...
        self._ssl = self._job.config['postgresql']['ssl']
        excludes = config.POSTGRESQL_DEFAULT_EXCLUDES
        excludes.extend(self._job.config['postgresql']['excludes'])
    ...
    def _get_databases(self):
        try:
            con = self._get_connection()
            cursor = con.cursor()
            cursor.execute(config.POSTGRESQL_SQL_GET_LIST_OF_DATABASES.format(excludes=self._excludes))
